      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50/50 hover:bg-gray-50/50">
            ...
            <TableHead className="min-w-[200px]">Offer Details</TableHead>
          ...
          {offers.length === 0 ? (
            ...
            offers.map((offer: any, index: number) => (
              <TableRow key={offer.offer_id || index} className="hover:bg-gray-50">
                ...
                {isColVisible("clicks") && <TableCell className="text-gray-600">{offer.clicks?.toLocaleString() || 0}</TableCell>}
                {isColVisible("views") && <TableCell className="text-gray-600">{offer.views?.toLocaleString() || 0}</TableCell>}
                {isColVisible("priority") && <TableCell className="text-gray-600">#{offer.priority || offer.sort_order || index + 1}</TableCell>}
                {isColVisible("status") && (
                  <TableCell>
