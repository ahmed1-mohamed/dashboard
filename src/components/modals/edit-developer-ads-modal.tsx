...
import { HelpCircle, Upload, ChevronDown, Search } from "lucide-react";
...
export function EditDeveloperAdModal({
  ad,
  isOpen,
  onClose,
  onSuccess,
  ...
  //      if (developerId) {
  //        try {
  //           const dev = await fetchDeveloperDetails(developerId, session.user.accessToken);
  ...
  //      // Developer is now set from session in separate useEffect
  ...
  //   if (!validateDateTimeRange(data.startDate, data.endDate)) return;
