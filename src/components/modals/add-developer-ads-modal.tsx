...
import { createAd, fetchDeveloperDetails } from "@/data/api-client";
import { useAddAdData } from "@/hooks/use-add-ad";
...
export function AddDeveloperAdModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateAdModalProps) {
  ...
  // const [developerName1, setDeveloperName] = useState<string>("");

  // const { data: session } = useSession();

  // // Fetch developer name from session's developer_id
  // useEffect(() => {
  //   const fetchDeveloper = async () => {
  //     if (!session?.user?.accessToken) return;

  //     const developerId =
  //       session.user.user_developer_relationship?.developer_id;
  //     if (developerId) {
  //       try {
  //         const dev = await fetchDeveloperDetails(
  //           Number(developerId),
  //           session.user.accessToken,
  //         );
  //         console.log("Fetched developer details:", dev);
  //         setDeveloperName(dev.data.name || "");
  //         setValue("developerId", developerId.toString());
  //         // Load projects and properties for this developer
  //         loadProjects(developerId);
  //         loadProperties(developerId);
  //       } catch (error) {
  ...
  // // Fetch projects when developer is selected
  // // const loadProjects = async (developerId: number, search?: string) => {
  // //   if (!session?.user?.accessToken) return;
  // //   // setProjectsLoading(true);
  // //   try {
  // //     const data = await fetchProjectsByDeveloper(
  // //       session.user.accessToken,
  // //       developerId,
  // //       1,
  // //       "100",
  ...
  // // Fetch properties when developer is selected
  // // const loadProperties = async (developerId: number, search?: string) => {
  // //   if (!session?.user?.accessToken) return;
  // //   setPropertiesLoading(true);
  // //   try {
  // //     const data = await fetchPropertiesByDeveloper(
  // //       session.user.accessToken,
  // //       developerId,
  // //       "active",
  // //       "100",
  ...
  // // Debounced project search effect
  // useEffect(() => {
  ...
  // // Debounced property search effect
  // useEffect(() => {
  //   const developerId = watch("developerId");
  ...
  //     const response = await createAd(formData, session.user.accessToken);
